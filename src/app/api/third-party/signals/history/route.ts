...
export async function GET(request: Request) {
  try {
    ...
    // 检查请求频率限制
    const rateLimitCheck = await checkRateLimit(apiKeyId, '/api/third-party/signals/history');
    ...
    await logApiRequest(apiKeyId, '/api/third-party/signals/history', clientIp);
    ...
    if (error) {
      console.error('查询历史信号数据失败:', error);
      return NextResponse.json({
        success: false,
        error: {
          code: 'DATABASE_ERROR',
    ...
    console.error('查询历史信号数据时发生错误:', error);
